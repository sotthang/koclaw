...
class ComputerUseTool(Tool):
    name = "computer_use"
    description = (
        "⚠️ 사용자가 '화면', '데스크탑', '클릭', '스크린샷', 'computer_use' 등 화면 제어를 명시적으로 언급한 경우에만 사용하세요. "
        "그 외 일반 작업(검색, 파일, 웹 등)에는 절대 사용하지 마세요. "
        "데스크탑을 제어합니다. "
        "브라우저 열기, 마우스 클릭, 텍스트 입력, 스크린샷, 셸 명령 실행 등을 자동화합니다. "
        "클릭/스크롤 좌표는 반드시 screenshot으로 찍은 이미지 기준으로 계산하세요 "
        "(서버가 자동으로 실제 화면 좌표로 변환합니다). "
