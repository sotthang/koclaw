...
한국형 오픈소스 AI 에이전트. Python, asyncio, SQLite, Slack Socket Mode / Discord / Telegram 기반.
...
| 파일 파싱 (PDF/DOCX/HWP/Excel/PPTX) | `koclaw/core/file_parser.py` |
| MCP 서버 연결 및 Tool 래핑 | `koclaw/core/mcp_loader.py` |
...
| Windows 파일 읽기 tool | `koclaw/tools/windows_file.py` |
...
- [ ] `koclaw/tools/{name}.py` — `Tool` 상속, `name` / `description` / `parameters` / `execute` 구현 (`needs_session_context = True` 이면 Agent가 `_session_id`를 자동 주입)
...
- [ ] `koclaw/channels/telegram.py` → `HELP_TEXT` 업데이트 (새 기능 설명 추가)
- [ ] `main.py` — `tools.register(NewTool())` 추가 (또는 `pyproject.toml` entry-points 등록)
- [ ] `README.md` 지원 도구 표 업데이트

### MCP 서버 추가 시

- [ ] `mcp_servers.json`에 서버 정의 추가 (`mcp_servers.json.example` 참고)
- [ ] 봇 재시작 — `koclaw/core/mcp_loader.py`가 시작 시 자동으로 연결하고 `ToolRegistry`에 등록
- [ ] tool 이름 충돌 주의 — MCP tool 이름이 기존 tool과 겹치면 로그 경고 후 덮어씀
...
**새 기능을 추가할 때마다 반드시 세 채널 모두 업데이트해야 합니다.**

| 채널 | 파일 | 형식 |
| --- | --- | --- |
| Slack | `koclaw/channels/slack.py` | Slack mrkdwn (`*굵게*`, `` `코드` ``) |
| Discord | `koclaw/channels/discord.py` | Discord markdown (`**굵게**`, `` `코드` ``) |
| Telegram | `koclaw/channels/telegram.py` | Telegram HTML (`<b>굵게</b>`, `<code>코드</code>`) |
...
- **Tool 세션 컨텍스트**: `needs_session_context = True` 로 설정하면 Agent가 `_session_id` 인자를 `execute()`에 자동 주입 (`computer_use` 등 세션별 상태가 필요한 tool에 사용)
...
- Slack/Discord/Telegram 봇은 특정 워크스페이스/서버/그룹 멤버만 사용 가능
...
> **중요**: `parse_memory_context`에서 `user_scope`를 채널 컨텍스트에도 활성화하면 개인 기억이 공개 채널에 노출될 수 있습니다. `user` scope는 반드시 DM(`D...`, `G...`, `discord:dm:...`, `telegram:dm:...`) 전용으로 유지하세요.
...
- **SSRF 리다이렉트 추적**: `browse`, `rss`, Discord/Telegram 파일 다운로더에서 리다이렉트 대상 URL 재검증 없음
...
- `.claude/settings.local.json`, `.claude/settings.json`, `.claude/memory/` 는 개인 설정 — 커밋하지 않음
- `.claude/skills/` 는 공유 스킬 — 커밋 대상 (`/test`, `/lint`, `/pr`, `/new-tool` 커맨드)
- `.claude/*.md` 는 공유 전략 문서 — 커밋 대상 (`github-strategy.md` 등, Claude 기반 기여자용)
- `storage/koclaw.db` — 운영 DB, 직접 수정 금지
- `mcp_servers.json` — API 토큰 등 민감 정보 포함 가능, `.gitignore`에 포함됨 — 커밋 금지
