...
환경변수:
    ...
    CHROMIUM_CDP_PORT      Chromium 원격 디버깅 포트 (기본값: 9222)
                           이 포트로 Chromium을 독립 프로세스로 실행해 서버 재시작 후에도
                           열려 있던 페이지가 유지됩니다.
"""
...
_CHROMIUM_CDP_PORT = int(os.environ.get("CHROMIUM_CDP_PORT", "9222"))
...
class BrowserCloseRequest(BaseModel):
    keep_alive: bool = True  # True: Playwright 연결만 끊고 Chromium 프로세스 유지
...
def _find_chromium() -> str | None:
    """시스템에 설치된 Chromium/Chrome 실행 파일 경로를 찾는다."""
    import shutil
...
def _launch_detached_chromium() -> None:
    """Chromium을 서버와 독립된 프로세스로 실행 (서버 재시작 후에도 유지)."""
    chromium_exe = _find_chromium()
    if not chromium_exe:
        logger.warning(
            "[browser] 시스템 Chromium을 찾지 못했습니다. Playwright 내장 브라우저로 폴백합니다."
        )
        return
    cmd = [
        chromium_exe,
        f"--remote-debugging-port={_CHROMIUM_CDP_PORT}",
        "--no-first-run",
    ...
    logger.info("[browser] 독립 Chromium 프로세스 시작 (CDP 포트: %d)", _CHROMIUM_CDP_PORT)
...
async def _ensure_browser():
    ...
    서버 재시작 후에도 페이지가 유지되도록 CDP로 기존 Chromium에 재연결을 먼저 시도한다.
    1) CDP 재연결 시도 → 기존 Chromium이 살아 있으면 열려 있던 페이지 그대로 복원
    2) 실패하면 Chromium을 독립 프로세스로 시작한 뒤 CDP 연결
    3) 최종 폴백: Playwright 내장 브라우저 (재시작 시 페이지 유지 안 됨)
    """
    global _pw_playwright, _pw_browser, _pw_page

    # 이미 연결된 페이지가 있으면 살아 있는지 확인
    ...
    cdp_url = f"http://localhost:{_CHROMIUM_CDP_PORT}"

    # 1) 이미 실행 중인 Chromium에 CDP로 재연결
    try:
        _pw_browser = await _pw_playwright.chromium.connect_over_cdp(cdp_url)
        contexts = _pw_browser.contexts
        if contexts and contexts[0].pages:
            _pw_page = contexts[0].pages[0]
            logger.info("[browser] 기존 Chromium 재연결 — 열려 있던 페이지 복원: %s", _pw_page.url)
        else:
            ctx = contexts[0] if contexts else await _pw_browser.new_context()
            _pw_page = await ctx.new_page()
            logger.info("[browser] 기존 Chromium 재연결 — 새 페이지 생성")
        return _pw_page
    except Exception:
        pass  # CDP 연결 실패 → 새로 시작

    # 2) 독립 프로세스로 Chromium 실행 후 CDP 재연결
    _launch_detached_chromium()
    await asyncio.sleep(2.0)  # Chromium 기동 대기
    try:
        _pw_browser = await _pw_playwright.chromium.connect_over_cdp(cdp_url)
        contexts = _pw_browser.contexts
        ctx = contexts[0] if contexts else await _pw_browser.new_context()
        pages = ctx.pages
        _pw_page = pages[0] if pages else await ctx.new_page()
        logger.info("[browser] 새 독립 Chromium 시작 후 CDP 연결 완료")
        return _pw_page
    except Exception:
        pass

    # 3) 최종 폴백: Playwright 내장 브라우저
    logger.warning(
        "[browser] CDP 연결 실패 — Playwright 내장 브라우저로 폴백 (재시작 시 페이지 유지 안 됨)"
    )
    try:
        _pw_browser = await _pw_playwright.chromium.launch(
            headless=False,
            args=[f"--remote-debugging-port={_CHROMIUM_CDP_PORT}"],
        )
    except Exception:
        await asyncio.to_thread(
            subprocess.run,
...
async def browser_close(req: BrowserCloseRequest = BrowserCloseRequest()):
    """브라우저 닫기.

    keep_alive=true(기본값): Playwright 연결만 끊고 Chromium 프로세스는 유지.
                            서버 재시작 후 CDP로 다시 연결 가능.
    keep_alive=false:        Chromium 프로세스까지 완전히 종료.
    """
    global _pw_playwright, _pw_browser, _pw_page
    _pw_page = None
    if req.keep_alive:
        # Playwright 연결 객체만 해제 — Chromium 프로세스는 계속 실행
        _pw_browser = None
    else:
        ...
        if _pw_playwright is not None:
            ...
            _pw_playwright = None
    return {"ok": True, "chromium_running": req.keep_alive}
